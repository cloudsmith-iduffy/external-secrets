...
| Key        | Description                                                                    |
| ---------- | ------------------------------------------------------------------------------ |
| registry   | Domain name of the Cloudsmith container registry (e.g., `docker.cloudsmith.io`). |
...
Use these values when configuring the OIDC service in your Cloudsmith organization settings.
...
- **Registry Domain**: The `registry` field will contain the appropriate Cloudsmith container registry domain for your organization.
...
- **Token Exchange Fails**: Verify that your OIDC service in Cloudsmith is correctly configured with your cluster's issuer and the service account's subject.
